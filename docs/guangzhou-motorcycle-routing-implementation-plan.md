...
flowchart TD
    ...
    K -->|是| M{附近有摄像头?}
    M -->|否| N[显示第一次路线]
    M -->|是| O[最多 5 次 route + exclude_locations]
    O --> P{最终路线仍靠近摄像头?}
    P -->|否| Q[显示最终路线和避开数量]
    P -->|是| R[回退首选路线并提示未找到绕行]
```
...
应用最多执行 5 次二次规划，并在每次返回后重新检查路线是否仍靠近首选路线命中的摄像头；如果 5 次后仍未避开，则回退到首选路线并明确提示，不把仍经过点位的路线标记为已避开。
...
Case B：在 Case A 路线上放入测试摄像头点，并最多执行 5 次 exclude_locations 规划
```
...
- Case B 不经过测试点附近道路。
- 如果连续 5 次仍经过测试点附近道路，回退到 Case A 并显示绕行失败。
